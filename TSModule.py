...
class FGLS(object): #Cochrane - Orcutt 
    def __init__(self, y, X, n_iter = 1, vce = 'robust'): 
        self.y = y
        self.X = X
        ...
        for j in range(n_iter): 
            ...
            self.rho = self.ARp.out.b[0] 
            if self.rho/self.ARp.out.SEb[0] <= 1.96: 
                break 
            self.y_lag = lag(self.y, 1).values * self.rho 
            self.diffy = pd.Series(self.y.values - self.y_lag, index = self.y.index, name = self.y.name) 
            print("Iteration: {0}, rho = {1:.4f}".format(j+1, self.rho))
            X_lag = lag(self.X.iloc[:,0], 1).values * self.rho 
            self.diffX = pd.DataFrame(self.X.values[:,0] - X_lag, index = self.X.index, columns = [self.X.columns[0]]) 
            if k > 1: 
                for i in range(1, k): 
                    X_lag = lag(self.X.iloc[:,i], 1).values * self.rho
